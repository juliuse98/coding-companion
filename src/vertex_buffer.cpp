...
VertexBuffer::VertexBuffer(const void* data, unsigned int size, int GlDrawType)
    : m_Size(size), m_GlDrawType(GlDrawType)
{
    assert(size > 0);
    // Else there will be an error called when the glBufferData call is made.
    assert(data != nullptr);
...
void VertexBuffer::SetData(
    const unsigned int offset, const unsigned int size, const void* data
)
{
...
void VertexBuffer::Resize(unsigned int newSize, const void* newData)
{
    ...
    GLCall(glBindBuffer(GL_ARRAY_BUFFER, m_RendererId));
    GLCall(glBufferData(GL_ARRAY_BUFFER, newSize, nullptr, m_GlDrawType)
    ); // Create new buffer storage
