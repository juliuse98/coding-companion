...
#include <filesystem> // Portable file handling
...
namespace cabbage {
    ...
    unsigned int Shader::CompileShader(unsigned int type, const std::string& source)
    {
        ...
        if (result == GL_FALSE)
        {
            ...
            glDeleteShader(id); // Important: Delete the shader to avoid leaks
            return 0;
        }
    ...
    ShaderProgramSource Shader::ParseShader(const std::string& filepath)
    {
        ...
        std::filesystem::path filePath1 = currentDirectory / filepath;

        std::ifstream stream(filePath1.string());

        if (!stream.is_open())
        {
            std::cerr << "Could not open: " << filePath1.string() << std::endl;
            return {}; // Return an empty ShaderProgramSource
        }
