      ...
      m_projection(glm::ortho(0.0f, 1920.0f, 1000.0f, 0.0f, -1.0f, 1.0f))
{
    ...
    m_textureSlotBuffer =
        std::make_unique<VertexBuffer>(m_textureIds.data(), sizeof(float) * 8);
    m_transformData.push_back(glm::mat4(1.0f));
    m_transformBuffer = std::make_unique<VertexBuffer>(m_transformData.data(),
                                                       sizeof(glm::mat4) * 1);
...
void SpriteRenderer::draw(coco::CCObject& object)
{
    ...
    m_textureIds[0] =
        m_textureManager.bindTexture(object.GetSprite()->GetTexutre());
    ...
    for (it = children.begin(); it != children.end(); it++)
    {
        ...
        m_textureIds[1] =
            m_textureManager.bindTexture((*it)->GetSprite()->GetTexutre());
        m_transformData.push_back((*it)->GetTransform().getModelMatrix());
    }
    m_transformBuffer->Bind();
    m_transformBuffer->Resize(sizeof(glm::mat4) * m_transformData.size(),
                              m_transformData.data());
    m_vertexArray.UpdateBuffer(*m_transformBuffer, m_transformBufferLayout);
    m_uvBuffer.Bind();
    m_uvBuffer.Resize(sizeof(cabbage::UVCoordinate) * m_uvData.size(),
                      m_uvData.data());
...
void SpriteRenderer::draw(std::vector<coco::CCObject*> objectsToRender)
{
    LOG(INFO) << "Start Draw";
    RenderBatch batch(m_vertexBuffer, m_vertexBufferLayout);
    batch.addCCObject(objectsToRender);
    batch.prepare();
